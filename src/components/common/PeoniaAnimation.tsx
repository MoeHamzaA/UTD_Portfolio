    ...
    import('p5').then(({ default: P5 }) => {
      ...
      const sketch = (p: p5) => {
        ...
        p.draw = function () {
          ...
          if (phase === 'interactive') {
            mInfX += ((p.winMouseX - p.windowWidth / 2) * 0.02 - mInfX) * 0.04;
            mInfY += ((p.winMouseY - p.windowHeight / 2) * 0.02 - mInfY) * 0.04;
          } else {
            mInfX *= 0.95;
            mInfY *= 0.95;
          }
        ...
        function drawPeony3D(
          ...
          if (cAlpha > 0)
            items.push({ rz: 0, tp: 'c', sx: 0, sy: 0, sa: 0, alpha: cAlpha });
          ...
          for (let layer = f.layers; layer >= 0; layer--) {
            ...
            const np =
              f.petalsPerLayer + p.floor(layer * 1.5) + p.floor(innerBoost * 5);
            ...
            for (let i = 0; i < np; i++) {
              ...
              let angle =
                (p.TWO_PI / np) * i + layer * 0.42 + p.sin(layer * 2.1) * 0.12;
              ...
              const rAmt =
                f.ruffleAmt * ebl * (1 + layerFall * 2 + innerBoost * 0.5) * ps;
